...
/**
 * Servlet implementation class YelpApiCall
 */
@WebServlet("/YelpApiCall")
public class YelpApiCall extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		double lat = 0.00;
		double lon = 0.00;
		if(allInfo[1].trim().contentEquals("nil")) {
			city = null;
		...
		if(!allInfo[2].contentEquals("nil")) {
			System.out.println("Triggered");
			lat = Double.parseDouble(allInfo[2]);
			lon = Double.parseDouble(allInfo[3]);
		}
		
		PrintWriter pw = response.getWriter();
		
		YelpAPIResponse res = new YelpAPIResponse();
		res.searchResults(term,city,lat,lon);
		List<Business> list1 = res.results.getBusinesses();
